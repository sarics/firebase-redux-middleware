...
var getVal = function getVal(obj) {
  return typeof obj === 'undefined' || obj === null ? obj : obj.valueOf();
};

var isEqual = function isEqual(obj1, obj2) {
  var obj1Val = getVal(obj1);
  var obj2Val = getVal(obj2);

  if (obj1Val === null || obj2Val === null || (typeof obj1Val === 'undefined' ? 'undefined' : _typeof(obj1Val)) !== 'object' || (typeof obj2Val === 'undefined' ? 'undefined' : _typeof(obj2Val)) !== 'object') {
    return obj1Val === null && obj2Val === null || Number.isNaN(obj1Val) && Number.isNaN(obj2Val) || obj1Val === obj2Val;
  }

  return Array.isArray(obj1Val) === Array.isArray(obj2Val) && Object.keys(obj1Val).length === Object.keys(obj2Val).length && Object.keys(obj1Val).reduce(function (isEq, key) {
    return isEq && isEqual(obj1Val[key], obj2Val[key]);
  }, true);
};
...
var getDeepData = function getDeepData(data) {
  ...
  if (data === null || (typeof data === 'undefined' ? 'undefined' : getDeepData__typeof(data)) !== 'object') return undefined;
...
var childrenHaveId = function childrenHaveId(arr) {
...
/* harmony default export */ var getDifference = (function (oldData, newData) {
  ...
  return newData.reduce(function (changes, item) {
    var newChanges = _extends({}, changes);
...
var snapshotToArray__extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

/* harmony default export */ var snapshotToArray = (function (snapshot) {
  var list = [];

  snapshot.forEach(function (childSnapshot) {
    list.push(snapshotToArray__extends({
...
var extendNewAction = function extendNewAction(newAction, key, newActionData) {
  return Object.assign(Array.isArray(newAction) ? [] : {}, newAction, _defineProperty({}, key, newActionData));
};
...
var addIdToActionData_addIdToActionData = function addIdToActionData(action, data, id) {
  ...
  return Object.keys(action).reduce(function (newAction, key) {
    ...
    if (found || !actionData || (typeof actionData === 'undefined' ? 'undefined' : addIdToActionData__typeof(actionData)) !== 'object') return extendNewAction(newAction, key, actionData);

    var newActionData = void 0;
    if (utils_isEqual(actionData, data)) {
      found = true;

      newActionData = addIdToActionData__extends({
        id: id
      }, data);
    } else {
